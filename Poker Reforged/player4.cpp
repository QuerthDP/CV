...
ifstream load2("card_players.txt");
	...
	int highest_card2 = -1;
		for (i = 0; i < 2; i++){
			if (player4[i] > highest_card2 && player4[i] < highest_card){
...
highest_card2 = card_value[highest_card2];	
	...
	if((p3_ready == -1 || p3_ready == 2) && (p1_ready == -1 || p1_ready == 2) && (p2_ready == -1 || p2_ready == 2) && p4_deposit == c){
		p4_ready = 2;
	}
...
if(p4_ready == 0){
	...
	cout << p4 << " wykonuje ruch..." << endl << endl;
	wait(3);
	
		int ps = 1, ch = 8, rs = 1;
	
	
	if(c <= 1.25*p4_deposit){
		ch = ch + 5;
	}
	if(highest_card < 7){
		ps = ps + 5;
	...
	if(c > 1.5*p4_deposit && c > p4_deposit + 1000){
		ps = ps + 10;
	}
	if(c <= p4_deposit){
		ps = 0;
	...
	p4_move = rand() % (ps+ch+rs) + 1; //test
	
//	cout << ps << " " << ch << " " << rs << endl;
//	cout << p4_move << endl;
	
	if(p4_move >= 0 && p4_move < ps){
		p4_move = 1;
	}
	if(p4_move >= ps && p4_move < ps+ch){
		p4_move = 2;
	}
	if(p4_move >= ps+ch && p4_move <= ps+ch+rs){
		p4_move = 3;
	}
	...
	if(p4_move == 2){
		if(p4_money <= c - p4_deposit){
			cout << p4 << " gra Va Banque!" << endl;
			p4_deposit = p4_deposit + p4_money;
			p4_money = 0;
			p4_ready = 2;
		}else{
			cout << p4 << " sprawdza." << endl;
			p4_money = p4_money + p4_deposit - c;
			p4_deposit = c;
			p4_ready = 1;
		}
	}
	if(p4_move == 3){
		...
		if(p4_money <= c - p4_deposit){
			...
			int p4_move2;
			p4_move2 = rand() % (p4_money+p4_deposit-c) + 1; //test
			...
			if(p4_move2 >= p4_money + p4_deposit-c){
				cout << p4 << " gra Va Banque!" << endl;
				p4_deposit = p4_deposit + p4_money;
				p4_money = 0;
				p4_ready = 2;
			}else{
				cout << p4 << " podbija o " << p4_move2 << "$." << endl;
				p4_money = p4_money + p4_deposit - (c + p4_move2);
				p4_deposit = c + p4_move2;
				p4_ready = 1;
			}
		}
	}
