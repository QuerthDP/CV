...
ifstream load2("card_players.txt");
	...
	int highest_card2 = -1;
		for (i = 0; i < 2; i++){
			if (player3[i] > highest_card2 && player3[i] < highest_card){
...
highest_card2 = card_value[highest_card2];
	...
	if((p4_ready == -1 || p4_ready == 2) && (p1_ready == -1 || p1_ready == 2) && (p2_ready == -1 || p2_ready == 2) && p3_deposit == c){
		p3_ready = 2;
	}
...
if(p3_ready == 0){
	...
	cout << p3 << " wykonuje ruch..." << endl << endl;
	wait(3);	
	...
	if(c > 1.5*p3_deposit && c > p3_deposit + 1000){
		ps = ps + 10;
	}
	if(c <= p3_deposit){
		ps = 0;
	...
	p3_move = rand() % (ps+ch+rs) + 1; //test
	
//	cout << ps << " " << ch << " " << rs << endl;
//	cout << p2_move << endl;
	
	if(p3_move >= 0 && p3_move < ps){
		p3_move = 1;
	}
	if(p3_move >= ps && p3_move < ps+ch){
		p3_move = 2;
	}
	if(p3_move >= ps+ch && p3_move <= ps+ch+rs){
		p3_move = 3;
	}
	...
	if(p3_move == 2){
		if(p3_money <= c - p3_deposit){
			cout << p3 << " gra Va Banque!" << endl;
			p3_deposit = p3_deposit + p3_money;
			p3_money = 0;
			p3_ready = 2;
		}else{
			cout << p3 << " sprawdza." << endl;
			p3_money = p3_money + p3_deposit - c;
			p3_deposit = c;
			p3_ready = 1;
		}
	}
	if(p3_move == 3){
		...
		if(p3_money <= c - p3_deposit){
			...
			int p3_move2;
			p3_move2 = rand() % (p3_money+p3_deposit-c) + 1; //test
			...
			if(p3_move2 >= p3_money + p3_deposit-c){
				cout << p3 << " gra Va Banque!" << endl;
				p3_deposit = p3_deposit + p3_money;
				p3_money = 0;
				p3_ready = 2;
			}else{
				cout << p3 << " podbija o " << p3_move2 << "$." << endl;
				p3_money = p3_money + p3_deposit - (c + p3_move2);
				p3_deposit = c + p3_move2;
				p3_ready = 1;
			}
		}
	}
